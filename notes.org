...
** <<<Wordpress The Loop>>>
*** Basic Syntax, <<<Tags used in The Loop>>>
#+BEGIN_EXAMPLE
<?php 
...
Used: WP_Meta_Query, WP Tax Query
#+BEGIN_EXAMPLE
$args = array(
...
** add_filter
#+BEGIN_EXAMPLE
add_filter( 'tagname', 'lili_funct'
, int $priority = 10 // lower number sooner
, int $accepted_args = 1 // default is 1! Be careful
)
...
a = a.map(function(i) { return i.toUpperCase(); });
...
** Enumerable
By default, all methods and properties are enumerable in javascript for in
